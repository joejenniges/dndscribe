...
type VoiceConnection struct {
	...
	// OnDAVETransition is called after a DAVE key transition completes.
	// Callers should re-derive receiver keys for all active users.
	OnDAVETransition func()
...
func (v *VoiceConnection) onEvent(ctx context.Context, binary bool, message []byte) {
	...
	if binary {
		...
		case 4: // udp encryption secret key
			...
			if !v.deaf {
				if v.OpusRecv == nil {
					// Buffer sized for ~5s at 4 speakers × 50pps = 1000 pps.
					// WHY: prevents UDP socket buffer overflow when the consumer
					// goroutine stalls briefly (GC, DAVE transition, disk I/O).
					// 128 was only ~640ms of headroom under load and let the UDP
					// reader block on channel send, cascading to kernel packet drops.
					v.OpusRecv = make(chan *Packet, 1024)
				}
...
func (v *VoiceConnection) opusReceiver(ctx context.Context) {
	...
	recvbuf := make([]byte, 4096) // Increased from 1024 for large DAVE-encrypted packets
	var nonce = make([]byte, v.cipher.NonceSize())
	...
	for {
		...
		if err != nil {
			...
			case <-ctx.Done():
				return
			default:
				// Log but don't kill the connection for transient UDP errors
				// (e.g. oversized DAVE packets). Only fatal if context is done.
				v.log(LogWarning, "udp read error (continuing): %v", err)
				continue
			}
		}
...
func (v *VoiceConnection) handleDAVEExecuteTransition(data json.RawMessage) {
	...
	if dave != nil {
		...
		// Notify application to re-derive receiver keys for all active users.
		if v.OnDAVETransition != nil {
			go v.OnDAVETransition()
		}
	}
}
